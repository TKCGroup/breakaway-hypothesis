...
- [ ] Confirm commit `512b953` is deployed with `SLACK_BOT_TOKEN` and `SLACK_CHANNEL_ID=C0AS8NB0LQY`, while keeping `DRY_RUN=true`.
- [ ] Verify `/run` still succeeds in dry-run mode after redeploy.
...
- [ ] Deploy `512b953` through Altbot under `from: breakaway`.
